...
async function startServer() {
  ...
  app.post('/api/search', async (req, res) => {
    try {
      const response = await fetch('https://ec-search-api-826846133648.asia-northeast1.run.app/api/search', {
        ...
        body: JSON.stringify({ keyword: req.body.query }),
      });
      if (!response.ok) {
  ...
  // ──────────────────────────────────────────────
  // Yahoo!ショッピング API エンドポイント
  // リクエスト: { query, results? }
  // レスポンス: Yahoo! V3 itemSearch のレスポンスをそのまま返す
  // ──────────────────────────────────────────────
  app.post('/api/yahoo', async (req, res) => {
    try {
      const { query, results = 10 } = req.body;
